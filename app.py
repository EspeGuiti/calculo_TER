...
if uploaded_file:
    ...
    if missing_columns:
        ...
        # ✅ CLEANING STEP: You need this here, before working with charges
        df["Ongoing Charge"] = df["Ongoing Charge"].str.replace("%", "").str.replace(",", ".").astype(float) / 100

        import numpy as np  # keep this if not already imported
...
# All unique options across the whole file (for the top bar)
...
for idx, family in enumerate(unique_families):
    ...
    def cascade_select(label, col_key, global_value, context_df, key_suffix):
        ...
        # Narrow context if valid
        if choice != "NOT FOUND":
            return choice, context_df[context_df[col_key] == choice]
        return choice, context_df
    # 0: Type of Share
    row_data["Type of Share"], context = cascade_select("Type of Share", "Type of Share",
                                                        global_filters["Type of Share"], context, 0)
    # 1: Currency
    row_data["Currency"], context = cascade_select("Currency", "Currency",
                                                   global_filters["Currency"], context, 1)
    # 2: Hedged
    row_data["Hedged"], context = cascade_select("Hedged", "Hedged",
                                                 global_filters["Hedged"], context, 2)
    # 3: Minimum Initial
    row_data["Minimum Initial"], context = cascade_select("Minimum Initial", "Minimum Initial",
                                                          global_filters["Minimum Initial"], context, 3)
    # 4: MiFID FH
    row_data["MiFID FH"], context = cascade_select("MiFID FH", "MiFID FH",
                                                   global_filters["MiFID FH"], context, 4)
    ...
    # Placeholders for later fill (Step 4)
    row_data["ISIN"] = ""
    row_data["Ongoing Charge"] = None
...
if st.button("Calculate TER"):
    ...
    for i, row in edited_df.iterrows():
        ...
        # Filter the Excel data using selected criteria
        match_df = df[
        ...
        # Pick the row with the lowest Ongoing Charge
